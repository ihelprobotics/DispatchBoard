...
function findPriceLine(lines, start) {
...
function sanitizeCustomerName(candidate) {
    const text = candidate?.trim();
    if (!text)
        return null;
    if (SOLD_BY_PATTERN.test(text))
        return null;
    const cleaned = text.replace(/\s+/g, " ").trim();
    const [firstSegment] = cleaned.split(/[,:;|]/);
    return firstSegment?.trim() || null;
}
function extractAddressCandidate(lines, idx, labelText) {
...
function findAddressBasedName(lines) {
...
function extractCustomer(lines, fallback) {
...
export async function parseInvoicePdf(buffer, fallbackCustomer) {
    ...
    const notesParts = [];
    if (orderNumber)
        notesParts.push(`order_number:${orderNumber}`);
    if (invoiceNumber)
        notesParts.push(`invoice_number:${invoiceNumber}`);
    return {
