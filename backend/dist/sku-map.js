export const skuDefinitions = [
    ...
    },
    {
        ...
        description: "High Intensity Reflective Tape - 3 Feet per Roll, Minimum Quantity of 3 Rolls, Designed for Precision in RPM Measurement and Superior Reflectivity",
    ...
    },
    {
        ...
        description: "Premium Rubber Tips for Tachometers - Set of 4 Durable Plastic Nose Tips for Precise Measurement and Protection",
...
const skuMatchers = skuDefinitions.map((entry) => ({
    entry,
    keywords: Array.from(new Set([entry.instrument, entry.description, ...(entry.aliases ?? [])]
        .map((value) => normalize(value))
        .filter(Boolean)))
}));
export function matchSku(line) {
...
export function findSkuByName(name) {
