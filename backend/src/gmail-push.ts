...
type ProcessMailParams = {
  mail: Awaited<ReturnType<typeof simpleParser>>;
  messageId: string;
  markAsSeen: () => Promise<void>;
};

type GmailPushOptions = {
  clientId: string;
  clientSecret: string;
  refreshToken: string;
  topicName: string;
  processMail: (params: ProcessMailParams) => Promise<void>;
  verificationToken?: string;
  watchRenewalIntervalMs?: number;
  maxUnreadFetch?: number;
};
...
export function createGmailPushService(options: GmailPushOptions) {
  const {
    clientId,
    clientSecret,
    refreshToken,
    ...
    processMail,
    verificationToken,
    watchRenewalIntervalMs = 6 * 60 * 60 * 1000,
    maxUnreadFetch = 25
  } = options;
  ...
  const pushHandler: RequestHandler = async (req, res) => {
